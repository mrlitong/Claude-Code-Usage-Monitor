...
We provide multiple ways to run the test suite:
...
python run_tests.py --quick     # Quick test without coverage
python run_tests.py --new       # Test only new features
```

#### 2. Using Shell script (Linux/macOS)
```bash
# Interactive mode
./run_all_tests.sh
...
make test-coverage  # Run with full coverage
make test-quick     # Quick test without coverage
make test-new       # Test only new features
make clean          # Clean test artifacts
```

#### 4. Direct pytest command
```bash
cd src
...
- Data aggregation (daily/monthly views)
- UI table rendering
